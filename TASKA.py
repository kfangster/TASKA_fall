...
class TASKA(AbstractBaseOutput):
    ...
    def __init__( self, com = 'COM3', mac = '88:6b:0f:0a:80:15' ):
        """
        Constructor
        ...
        for i2c in [ 11, 12, 13 ]:
            for motor in [ 1, 2 ]:
                ...
                self._ser.write( pkt )
                resp = self._ser.read( size = 6 ) # expected response packet: [ 64, 77, I2C, 6, 11, CHKSUM ]
    ...
    def _move_finger_single( self, finger, speed, position, amps = 10, stall = 20 ):
        ...
        self._ser.write( pkt )
        self._ser.read( size = 5 )

    def _move_finger_group( self, positions, speeds, amps = 10, stall = 20 ):
        """
        Parameters
        ----------
    ...
    def encoders(self):
        ...
        for i2c in [ 11, 12, 13 ]:
            ...
            # send / receive info
            self._ser.write( pkt )
            recv_1 = self._ser.read( size = 21 )
            recv_2 = self._ser.read( size = 26 )
...
if __name__ == '__main__':
    ...
    class_init = inspect.getargspec( TASKA.__init__ )
